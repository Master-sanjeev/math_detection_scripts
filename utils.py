...
def visualize_boxes_and_labels_on_image_array(
  ...
  inline_boxes = []
  displayboxes = [] # will contain the display math boxes in format [pg_no, x_mid, y_mid, box_width, box_height]
  ...
  for box, color in box_to_color_map.items():
    ...
    displayboxes.append([pg_no, (xmin + xmax)/2, (ymin + ymax)/2, xmax - xmin, ymax - ymin])
    # count += 1
    if instance_masks is not None:
